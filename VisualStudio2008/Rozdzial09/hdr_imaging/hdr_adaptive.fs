#version 150 
// hdr_adaptive.fs
// Wykonuje adaptacyjne odwzorowanie tonów na podstawie wzglêdnego po³o¿enia pikseli
// 
...
{ 
    vec4 hdrSample[25];
    for (int i = 0; i < 25; i++)
    {   // Wykonanie 25 operacji wyszukiwania wokó³ bie¿¹cego teksela  
		hdrSample[i] = texture(textureUnit0, vTexCoord.st + tc_offset[i]); 
	}
	
	// Obliczanie wa¿onego koloru obszaru
    ...
    // Obliczanie luminancji dla ca³ego j¹dra filtru
	float kernelLuminance = dot(kernelcolor.rgb, vec3(0.3, 0.59, 0.11));

	// Znajdowanie odpowiedniego naœwietlenia
